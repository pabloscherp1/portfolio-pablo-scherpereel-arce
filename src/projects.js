const projects = [
  {
    ...
    overview: "Endurance is NASA's proposed flagship rover for sustained exploration of the lunar South Pole–Aitken basin: a 2,000 km traverse through permanently shadowed terrain where Earth communication windows are sparse and ground-directed operations are impractical. The rover needs to autonomously schedule drives, science sampling, downlinks and thermal management over multi-day horizons, replan in response to faults, and do so within tight power and data constraints.\n\nThis Master's thesis at NASA JPL implements a MEXEC-analogous autonomy layer from scratch in MATLAB within MissionSim, JPL's internal mission-level simulation framework. The core contribution is a greedy receding-horizon task planner: a priority-ordered scheduler that projects resource timelines (power, data, thermal) across a multi-day horizon and places each task at its earliest feasible slot subject to temporal and resource constraints — the same architecture used onboard JPL spacecraft via MEXEC. The planner closes a full observe → plan → commit → execute → replan loop, replanning on task completion, fault detection, or projected resource violations.\n\nFour autonomy levels are benchmarked through full-mission Monte Carlo evaluation: Scripted (pre-computed ground timeline), Reactive (threshold-based state machine, already in MissionSim), Proactive (the greedy receding-horizon planner), and Adaptive (probabilistic fault recovery). The primary deliverable is not the software — it is the quantitative comparison. The results directly inform JPL's V&V investment in flight-certifying the onboard autonomy stack: if Reactive nearly matches Proactive in mission efficiency, JPL can redirect the enormous cost of certifying MEXEC for flight; if Proactive meaningfully outperforms Reactive, that justifies the investment. The sensitivity analysis in Phase 5 further characterises the mission conditions under which proactive planning yields meaningful gains — a generalisable result for any future long-range rover mission.",
